CREATE DATABASE IF NOT EXISTS COLLEGE_UNI;
USE COLLEGE_UNI;
...
INSERT INTO FacilityType (TypeName) VALUES
('Classroom'),
('Laboratory'),
('Auditorium'),
('Sports Complex'),
...
BEGIN
    ...
    WHERE FacilityID = NEW.FacilityID
      AND DATE(NEW.StartTime) = MaintenanceDate;
    
    IF m_count > 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Cannot reserve facility under maintenance on this date.';
    END IF;
END$$
...
BEGIN
    ...
    WHERE FacilityID = NEW.FacilityID
      AND DATE(StartTime) = NEW.MaintenanceDate;
END$$
